...
(function () {
	...
	class PedCalc {
		...
		bindEvents() {
			const on = (el, event, handler) => { if (el) el.addEventListener(event, handler.bind(this)); };
			...
			// Quick buttons
			this.quickBtns.forEach(btn => { on(btn, 'click', () => this.quickSelect(btn)); });

			// View toggle
			this.viewBtns.forEach(btn => { on(btn, 'click', () => this.toggleView(btn)); });
			...
			on(this.els.settingsModal, 'click', (e) => { if (e.target === this.els.settingsModal) this.hideSettings(); });
		...
		initKeyboardShortcuts() {
			document.addEventListener('keydown', (e) => {
				...
				if (ctrl) {
					switch (e.key) {
						case 's':
							e.preventDefault(); this.exportData(); break;
						case 'p':
							e.preventDefault(); this.printData(); break;
						case 'c':
							if (!window.getSelection().toString()) {
								e.preventDefault(); this.copyData();
							}
							break;
						case 'd':
		...
		showSettings() { if (this.els.settingsModal) this.els.settingsModal.classList.add('show'); }
		hideSettings() { if (this.els.settingsModal) this.els.settingsModal.classList.remove('show'); }
...
${result.warning && this.state.showWarnings ? `<div class="drug-warning">⚠️ ${result.warning}</div>` : ''}`;
			} else {
				div.innerHTML =
...
<div class="drug-dose-display">
	...
	<span class="dose-unit">ml</span>
		...
		animateValue(element, newValue) {
			...
			if (oldValue !== newValue) {
				...
				setTimeout(() => { element.classList.remove('updating'); }, CONFIG.animation.normal);
			}
		}
	...
	function initApp() {
		...
		window.addEventListener('online', () => { if (window.pedCalc) window.pedCalc.toast('✅ Online'); });
		window.addEventListener('offline', () => { if (window.pedCalc) window.pedCalc.toast('⚠️ Offline'); });
