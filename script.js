// ===== Pediatrics Calculator - Compact Version =====
// Version: 2.1.0
// Optimized for compact UI with enhanced performance
...
(function() {
  ...
  const DRUGS = [
    {
      ...
      times: 3,
      color: '#FFE0E6',
      grid: [1.5, 2, 2.5, 3, 4, 4, 5, 5]
    },
    {
      id: 'augm',
      name: 'Augmentin',
      strength: '400/57mg/5ml',
      times: 2,
      color: '#E6F7FF',
      grid: [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5]
    },
    {
      id: 'cotr',
      name: 'Cotrimoxazole',
    ...
    },
    {
      ...
      minMonths: 6,
      color: '#F0E6FF',
      grid: [1.5, 2.5, 3, 4, 4.5, 5.5, 6, 7.5]
    },
    {
      id: 'salb',
      name: 'Salbutamol',
    ...
    },
    {
      ...
      times: 4,
      color: '#FFEBE6',
      grid: [2.5, 4, 5, 6.5, 7.5, 9, 10, 12.5]
    },
    {
      id: 'domp',
      name: 'Domperidone',
    ...
    },
    {
      ...
      times: 2,
      color: '#F5E6D3',
      grid: [0.6, 1, 1.2, 1.5, 1.8, 2, 2.4, 3]
    }
  ];
  ...
  class PedCalc {
    constructor() {
      ...
      this.els = {}; // DOM elements cache
      this.timers = {}; // Timeout/interval storage
      this.init();
    }
    ...
    cacheDOMElements() {
      // Use querySelector for better performance with IDs
      ...
      // Cache query selectors for repeated use
    ...
    bindEvents() {
      ...
      // Patient inputs - with debouncing for performance
    ...
    calculateIV() {
      ...
      const deficitHourly = this.state.deficitHours > 0 ?
        deficitTotal / this.state.deficitHours : 0;
    ...
    updateAge() {
      ...
      this.els.ageYears.textContent = `= ${text}`;
      this.els.statusAge.textContent = text;
    }
    ...
    updateStatus() {
      this.els.statusWeight.textContent = `${this.state.weight} kg`;
      this.els.roundingStatus.textContent =
        this.state.roundingMode === 'nearest' ? 'ปัดใกล้สุด' : 'คำนวณก้ำกึ่ง';
    }
    ...
    createDrugElement(drug, result, index) {
      ...
      const dose = result.dose.toFixed(1);
    ...
    generateSummary() {
      const iv = this.calculateIV();
      const age = this.els.statusAge.textContent;
      ...
      text += `วิธีปัด: ${this.els.roundingStatus.textContent}\\n`;
      text += `─────────────────\\n`;
